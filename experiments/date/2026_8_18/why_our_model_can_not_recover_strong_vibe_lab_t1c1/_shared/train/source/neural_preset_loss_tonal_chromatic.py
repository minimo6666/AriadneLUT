...
class TonalChromaticStage1Objective(nn.Module):
    ...
    def __init__(
        ...
        lambda_vivid_tail: float = 0.0,
        vivid_tail_quantile: float = 0.90,
        ...
        self.lambda_vivid_tail = float(lambda_vivid_tail)
        self.vivid_tail_quantile = float(vivid_tail_quantile)
        if not 0.0 < self.vivid_tail_quantile < 1.0:
            raise ValueError("vivid_tail_quantile must be strictly between 0 and 1")
        self.l1 = nn.L1Loss()
        self.mse = nn.MSELoss()
    ...
    def _vivid_tail_ab_loss(
        self,
        output: torch.Tensor,
        target: torch.Tensor,
    ) -> torch.Tensor:
        """Match Oklab a/b only on each GT image's high-chroma tail.

        The mask is target-derived and detached. This does not reward generic
        saturation: the selected pixels must match the target a/b values.
        """
        output_lab = self._lab(output)
        target_lab = self._lab(target)
        target_chroma = torch.sqrt(
            target_lab[:, 1].square() + target_lab[:, 2].square() + 1e-12
        )
        batch = int(target_chroma.shape[0])
        flat_chroma = target_chroma.detach().reshape(batch, -1)
        thresholds = torch.quantile(
            flat_chroma,
            self.vivid_tail_quantile,
            dim=1,
            keepdim=True,
        )
        mask = (flat_chroma >= thresholds).to(dtype=output_lab.dtype)
        ab_error = (
            output_lab[:, 1:3] - target_lab[:, 1:3]
        ).abs().mean(dim=1).reshape(batch, -1)
        per_image = (ab_error * mask).sum(dim=1) / mask.sum(dim=1).clamp_min(1.0)
    ...
    def forward(self, result: dict, image_a: torch.Tensor, image_b: torch.Tensor) -> dict:
        ...
        if self.lambda_vivid_tail > 0.0:
            vivid_ab = self._vivid_tail_ab_loss(result["output_ab"], image_b)
            vivid_ba = self._vivid_tail_ab_loss(result["output_ba"], image_a)
            vivid_tail = vivid_ab + vivid_ba
        else:
            vivid_ab = reconstruction.new_zeros(())
            vivid_ba = reconstruction.new_zeros(())
            vivid_tail = reconstruction.new_zeros(())
        ...
        total = (
            ...
            + self.lambda_vivid_tail * vivid_tail
        )
        ...
        return {
            ...
            "gamut_loss": gamut,
            "vivid_tail_a_to_b": vivid_ab,
            "vivid_tail_b_to_a": vivid_ba,
            "vivid_tail_loss": vivid_tail,
            "total": total,
        }
