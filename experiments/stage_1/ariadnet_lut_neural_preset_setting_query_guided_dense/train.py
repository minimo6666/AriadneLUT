...
from ariadne_query_guided_dense import AriadneLUTQueryGuidedDense  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train Query-Guided Dense Ariadne in the matched Stage-1 setting."
        )
    )
...
def unwrap_model(model: nn.Module) -> AriadneLUTQueryGuidedDense:
    unwrapped = model.module if isinstance(model, DistributedDataParallel) else model
    if not isinstance(unwrapped, AriadneLUTQueryGuidedDense):
...
def save_checkpoint(
    ...
    payload = {
        ...
        "protocol": "dense_stage1_plus_explicit_query_conditioning_only",
...
def load_training_checkpoint(
    path: Path,
    model: AriadneLUTQueryGuidedDense,
    optimizer: torch.optim.Optimizer,
    scheduler,
    scaler: GradScaler,
    device: torch.device,
...
def write_manifest(
    ...
    payload = {
        "protocol": "Dense Stage-1 plus query conditioning only",
        ...
        "best_criterion": "min validation total loss (matched to Stage-1 baseline)",
    }
...
def validate(
    ...
    for batch in tqdm(
        iterator,
        desc=f"ValQueryDense {epoch:03d}",
    ...
    if is_main and first_visual is not None:
        save_labeled_grid_v2(
            ...
            title=f"Query-guided Dense Ariadne - epoch {epoch}",
...
def main() -> None:
    ...
    base_model = AriadneLUTQueryGuidedDense(cfg.model, query_cfg=dict(cfg.query)).to(device)
    criterion = NeuralPresetObjectiveForAriadne(
    ...
    decoder_parameters = sum(
        parameter.numel()
        for name, parameter in base_model.named_parameters()
        if name.startswith("normalization_lut_generator") or name.startswith("style_lut_generator")
    )
    ...
    rank_print(
        ...
        f"lut_decoders={decoder_parameters:,}; query_params={query_parameters:,}"
    )
    rank_print(
    ...
    try:
        for epoch in range(start_epoch, int(cfg.train.epochs)):
            ...
            progress = tqdm(
                iterator,
                total=total_batches,
                desc=f"TrainQueryDense {epoch:03d}",
                disable=not is_main,
            )
            ...
            for batch_index, batch in enumerate(progress):
                ...
                if (
                    ...
                    save_labeled_grid_v2(
                        ...
                        title=f"Query-guided Dense optimizer step {global_step}",
                        max_items=int(cfg.validation.max_visual_items),
                    )
            ...
            if is_main:
                ...
                current_metric = float(val_values["total"])
                ...
                rank_print(
                    f"Checkpoint policy OK: {checkpoint_names}; best_validation_total={best_metric:.6f}",
                    flush=True,
                )
            if distributed:
                dist.barrier()
